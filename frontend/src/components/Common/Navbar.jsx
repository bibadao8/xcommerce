    ...
    return <>
        <nav className='bg-[#d8cfbc] container mx-auto flex items-center justify-between py-4 px-6'>
            ...
            <div className='flex items-center space-x-4'>
                <Link to='/admin' className='block bg-black px-4 py-2 rounded-2xl text-sm text-white hover:bg-gray-800 transition-all duration-200 '>Admin</Link>
                <Link to="/profile" className='hover:text-black'>
                    <HiOutlineUser className='h-6 w-6 text-gray-700' />
                </Link>
                <button onClick={toggleCartDrawer} className='relative hover:text-black'>
                    <HiOutlineShoppingBag className=' hover:cursor-pointer h-6 w-6 text-gray-700' />
                    {cartItemCount > 0 && (<span className='absolute -top-1 bg-[#ac3b61] text-white text-xs rounded-full px-2 py-0.5'>{cartItemCount}</span>)}
                </button>
